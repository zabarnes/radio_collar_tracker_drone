...
// Settings
int sysid =
    42;             ///< The unique system id of this MAV, 0-127. Has to be consistent across the system
int compid = 110;
int serial_compid = 0;
...
uint8_t    ap_fixtype =
    3;               //   0= No GPS, 1 = No Fix, 2 = 2D Fix, 3 = 3D Fix
 ...
 */
void serial_read(int serial_fd) {
	int fd = serial_fd;
	bool gotGPS = 0;
		...
		if (msgReceived) {
			...
			switch (message.msgid) {
				...
				case MAVLINK_MSG_ID_GPS_RAW_INT: { // 24
						...
						//Serial.println("hola");
						ap_fixtype = mavlink_msg_gps_raw_int_get_fix_type(
						                 &message);                         // 0 = No GPS, 1 =No Fix, 2 = 2D Fix, 3 = 3D Fix``
						ap_sat_visible =  mavlink_msg_gps_raw_int_get_satellites_visible(
						                      &message);          // numbers of visible satelites
						...
						if (ap_fixtype == 3) {
							...
							ap_gps_altitude = mavlink_msg_gps_raw_int_get_alt(
							                      &message);                       // 1m = 1000
						}
						break;
					}
				...
				case MAVLINK_MSG_ID_VFR_HUD: { //  74
						ap_airspeed = 0;
						ap_groundspeed = mavlink_msg_vfr_hud_get_groundspeed(
						                     &message);                        // 100 = 1m/s
						ap_heading = mavlink_msg_vfr_hud_get_heading(
						                 &message);                                // 100 = 100 deg
						ap_throttle = mavlink_msg_vfr_hud_get_throttle(
						                  &message);                              // 100 = 100%
						ap_bar_altitude = mavlink_msg_vfr_hud_get_alt(&message) *
						                  100;                         // m
						ap_climb_rate = mavlink_msg_vfr_hud_get_climb(&message) *
						                100;                           // m/s
					}
					break;
...
int return_long() {return ap_longitude;}

int return_lati() {return ap_latitude;}

int return_alti() {return ap_gps_altitude;}
...
int init_serial() {
	...
	const char *commandline_usage =
	    "\tusage: %s -d <devicename> -b <baudrate> [-v/--verbose] [--debug]\n\t\tdefault: -d %s -b %i\n";
